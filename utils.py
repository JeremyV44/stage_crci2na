...
msig = Msigdb()
gmt = msig.get_gmt(category = "c8.all")
...
def create_volcano_plot(results_df, ligne, t,de_dir, xlim=None):
    ...
    # Enregistrer le plot
    filename = f"Volcano_plot_{ligne}_{t}_VS_{ligne}_ctrl"
    if xlim is not None:
...
# Même philosophie que la fonction précédente
    ...
    # Enregistrer le plot
    filename = f"Volcano_plot_{ligne}_{t}_VS_{ligne}_ctrl.png"
...
def enr_function(df_expressed,background,ligne,t,enrch_dir,mode, organism):
    ...
    if mode == "time":
        enr_res = gp.enrichr(gene_list =list_genes,background=liste_background, gene_sets = ["GO_Biological_Process_2023", f"KEGG_2019_{organism}"], organism=organism)
    else:
        enr_res = gp.enrichr(gene_list =list_genes,background=liste_background, gene_sets = ["GO_Biological_Process_2023", f"KEGG_2019_{organism}"], organism=organism)
...
def gsea_function(df,ligne,t,gsea_dir,mode, organism):
    ...
    try :
        if mode == "time":
            gsea_res = gp.prerank(rnk = df_rank, gene_sets = ["GO_Biological_Process_2023", f"KEGG_2019_{organism}"],permutation_num = 1000,organism=organism)
        else:
            gsea_res = gp.prerank(rnk = df_rank, gene_sets =["GO_Biological_Process_2023", f"KEGG_2019_{organism}"],permutation_num = 1000, min_size=3, max_size=1000, organism=organism)
        out_list = []
...
def process_differential_analysis(dds, lignees, temps, output_dir,mode, organism, uniprot_number):
    ...
    for ligne in lignees:
        ...
        for t in temps:
            ...
            stat_res.results_df = stat_res.results_df.sort_values(by='padj')
            biological_processes = ["N/A"] * len(stat_res.results_df)
            cellular_components = ["N/A"] * len(stat_res.results_df)
            summaries = ["N/A"] * len(stat_res.results_df)

            # Récupérer les informations pour chaque gène avec padj < 0.90
            # Filtrer les gènes avec padj < 0.05 et prendre les 200 premiers
            filtered_genes = stat_res.results_df[stat_res.results_df['padj'] < 0.05].head(uniprot_number).index.tolist()
            for gene in filtered_genes:
                if organism == "Mouse":
                    info = test_api.get_uniprot_data(gene, "10090")
                else:
                    info = test_api.get_uniprot_data(gene, "9606")
                if info:
                    try:
                        keywords_info = test_api.extract_keywords_info(info)
                        idx = stat_res.results_df.index.get_loc(gene)  # Obtenir l'index
                        biological_processes[idx] = ", ".join(keywords_info["biological_process"])
                        cellular_components[idx] = ", ".join(keywords_info["cellular_component"])
                        summaries[idx] = keywords_info["comment_value"]
                    except TypeError:
                        # Les listes sont déjà initialisées avec "N/A", donc pas besoin de faire autre chose ici
                        pass

            # Ajouter les nouvelles colonnes au DataFrame
            stat_res.results_df['Biological Processes'] = biological_processes
            stat_res.results_df['Cellular Components'] = cellular_components
            stat_res.results_df['Summary'] = summaries
            ...
            enr_function(sig_genes,stat_res.results_df,ligne,t,enrch_dir,mode,organism)

            stat_res.results_df = stat_res.results_df[(stat_res.results_df['padj'] < 0.90)]
            gsea_function(stat_res.results_df,ligne,t,gsea_dir,mode,organism)
